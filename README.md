...
- 「今日」根据合作价值和截止时间安排下一步行动，并集中呈现 Momcozy 主动邀请数量与品牌亲密度。
- 「合作机会」提供可解释选品，并在申请对外发送前要求红人确认。
- 「爆款创作」把账号信号、评论需求与品牌 Brief 转成内容切角和分镜脚本。
- 「AI 视频」演示素材选择、成片配置、生成与保存流程。
- 「数据复盘」同时分析流量、互动、购买意向和佣金信号。
- 「AI 经纪人」通过服务端 DeepSeek Responses API 连接模型，结合当前页面、商品、申请状态与最近对话，实时回答选品、合作、创作和复盘问题。
...
默认模型为 `deepseek-v4-flash`，可通过 `DEEPSEEK_MODEL` 覆盖。部署时应把 `DEEPSEEK_API_KEY` 配置为托管环境的服务端 Secret。
